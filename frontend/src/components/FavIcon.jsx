...
const FavIcon = ({ displayAlert, selected, inTopNavigation }) => {
  const originalFill = selected ? "#C80000" : "#EEEEEE";
  const fill = inTopNavigation ? "#C80000" : originalFill;
  ...
  return (
    <svg
      ...
      className={`fav-icon ${selected ? 'selected' : ''}`}
    >
      <path
        fill={fill}
      ...
      />
      {displayAlert && (
        <circle cx="20" cy="3" r="3.75" fill="#FFFF00" stroke="#C80000" strokeWidth="0.5" />
      )}
    </svg>
  );
};
